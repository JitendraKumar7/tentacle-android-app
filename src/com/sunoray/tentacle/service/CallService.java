...
public class CallService extends Service {
	...
	private void handleStart(Intent intent, int startId) {
		try {
			log.info("Service Strated. ID: "+ startId);
			if (intent != null) {
				...
				this.rec.setAudioSrc(PreferanceUtil.getSharedPreferences(context, PreferanceUtil.AudioSource, "0"));
				this.recordingOption = PreferanceUtil.getSharedPreferences(context,	PreferanceUtil.RecordingOption, "0");
			...
			log.debug("Excepition in CallService.onStart: ",e);
	...
	private class PhoneCallListener extends PhoneStateListener {	
		...
		public void onCallStateChanged(int state, String incomingNumber) {
			...
			try {
				log.debug("CallListner new state received: "+state);
				...
				if (TelephonyManager.CALL_STATE_OFFHOOK == state) {
					...
					try {
						...
						log.debug("Expeption in call listner state: ",e);
					}
				}
				...
				AppProperties.isCallServiceRunning = false;
				isPhoneCalling = false;
				log.debug("Expeption (onCallStateChanged): ",e);
			}
		}
	}
	...
	public void onDestroy() {
		log.debug("Call Service is Stoped");
		closeTDialer();
		AppProperties.isCallServiceRunning = false;
